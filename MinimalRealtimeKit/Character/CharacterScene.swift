...
//  T3.1 — the SMALLEST believable version of the character: ONE procedural orb + TWO eyes.
//  It listens (draws IN), speaks (swells OUT + smiles on loud peaks), and reacts (blink, breath,
//  eased posture per state). Pure SpriteKit + Core Graphics — no sprite sheets, no assets, no deps.
//
//  Adapted from `snippets/spritekit/MinimalCharacterScene.swift` (itself distilled from Pebbles'
//  1416-line PebblesScene). CUT for v1: the 9-stone cairn, springs, physics poke/tumble, drag,
//  happy-hop, react glance, sleep "z", and the tone system.
//
//  ── THE GOLDEN RULE (SPEC N6) ─────────────────────────────────────────────────────────────────
//  Amplitude is PULLED once per frame via `levelProvider`, inside SpriteKit's own render loop. It is
//  NEVER pushed through SwiftUI @State / @Observable / UIKit updateProperties — only the DISCRETE
//  state goes through observation (the VC calls `apply(state:)`). A 60Hz signal through the view
//  graph would thrash layout every frame.
//
//  N4: the scene NEVER owns audio or the session — it only READS a level closure and a discrete
//  state. There is no `deinit` and nothing here touches the realtime manager.
...
final class CharacterScene: SKScene {
    ...
    /// INPUT B — live mic / output level, 0…1, sampled EVERY FRAME (N6). Bound once by the VC.
    var levelProvider: () -> Float = { 0 }

    /// Honor Reduce Motion: freeze breath/voice-flutter, keep only gentle eases.
    var reduceMotion = false

    /// Fired when the orb is poked (so the host can add haptics, etc.). Optional.
    var onPoke: () -> Void = {}
    ...
    // One-shot tap squash envelope (0→1→0), laid on top so the per-frame scale can't stomp it.
    ...
    /// Rest center: horizontally centered, in the upper-middle (captions/controls live below it).
    private var restCenter: CGPoint { CGPoint(x: size.width / 2, y: size.height * 0.55) }

    // Textures generated ONCE (no per-frame Core Graphics on the live view).
    private static let bodyTexture = makeBodyTexture()
    private static let eyeTexture = makeEyeTexture()

    // Calm oval vs. happy "cheeks-up" crescent.
    ...
    private func build() {
        ...
        // A soft contact shadow grounds the orb so it doesn't float.
    ...
    // MARK: State application (INPUT A — DISCRETE, driven from observation, NOT per frame)

    /// Drive the look from the app's discrete state. Called by the VC's `updateProperties()` (and on
    /// first appear). The per-frame `update` eases toward whatever pose this sets — no jumps.
    func apply(state: PebblesState) {
        look = CharacterLook.make(state: state)
        if reduceMotion {
            look.breath = 0
            look.bounce = 0
        }
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        // Pull live amplitude ONCE, here, off observation (N6).
        let amp = CGFloat(max(0, min(1, levelProvider())))

        advancePokeSquash(dt: dt)

        // Always-present life: a slow breath sinusoid.
        ...
        // Ease the body toward its target pose (lerp -> no jumps).
        ...
        // Scale with a volume-preserving squash, plus the transient tap squash.
    ...
    /// Tap squash envelope: a smooth 0→1→0 over ~0.42s.
    ...
    private func applyEyes(dt: TimeInterval, t: TimeInterval, amp: CGFloat) {
        ...
        // Smile: briefly on loud speaking peaks (v1 has no sustained/playful tone).
        if look.crescentOnPeak {
        ...
        // Asymmetric blink: fast close (70ms), slow open (210ms).
        ...
        // Gentle idle look-around (open rest only).
        let lookAround = (look.eyes == .open) ? CGFloat(sin(t * 0.5)) * 2 : 0

        for (sign, eye) in [(-1.0, leftEye), (1.0, rightEye)] {
            // Whisper-quiet hand-made asymmetry: right eye a hair lower/smaller.
    ...
    /// Swap eye geometry only on transition: filled oval <-> stroked crescent.
    ...
    // MARK: Stage seam (hit-test + poke) — interaction is driven by the VC, not the scene

    /// True when a view-space point lands on the orb (with a generous touch margin). The VC's
    /// reversed-priority `hitTest` calls this to claim a precise orb tap for the SKView.
    func stoneHit(atViewPoint viewPoint: CGPoint) -> Bool {
        guard view != nil else { return false }
        let scenePoint = convertPoint(fromView: viewPoint)
        let dx = scenePoint.x - body.position.x
        let dy = scenePoint.y - body.position.y
        let radius = (Self.bodyDiameter / 2) * max(body.xScale, body.yScale) + 28   // touch margin
        return (dx * dx + dy * dy) <= radius * radius
    }

    /// Poke the orb: a transient squash (+ the optional `onPoke` hook). Driven by the VC's tap
    /// recognizer because the scene's own touch handling is disabled (`isUserInteractionEnabled = false`).
    func poke() {
        onPoke()
        pokeSquashAge = 0
    }

    // MARK: Procedural textures (generated once; warm "ceramic" palette)
    ...
    /// A soft radial blob (white core -> transparent) for the contact shadow.
...
// MARK: - Tiny inlined helper (a hex UIColor init, renamed to avoid collisions)
