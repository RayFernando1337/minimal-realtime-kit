...
//  T4.2 — stat_card.v1: show-only, but with a STRUCTURED / NESTED payload.
//
//  A `chip` + a `modules` array of tiles + real validation discipline (require `metric`, trim
//  everything, drop half-empty tiles, cap at 4) + a lenient `tone` → color map. Demonstrates
//  rich typed decode beyond a flat note. NON-interactive, so it reads nothing from
//  `FactoryContext`. Uses system fonts/colors + `.thinMaterial` (no app design tokens).
...
extension StatCardPayload {
    /// The ONE hard requirement is a non-empty `metric` (the hero number) — without it there's
    /// nothing to show, so this returns nil and the builder throws → the fallback. Everything
    /// else is lenient: trims all text, drops any module tile missing a label OR value, caps
    /// tiles at 4, collapses empty strings / a text-less chip to nil.
...
struct StatCardComponentView: View {
    ...
    /// Map a lenient `tone` hint to a color. Anything unknown/absent reads as the accent, so a
    /// garbled tone never breaks the skin.
