//
//  ChoiceComponent.swift  (REDACTED / adapted for minimal-realtime-kit)
//  Source: OpenAIRealtimeSample/Factory/ChoiceComponent.swift
//    - ChoicePayload.validated() ......... :27-50
//    - ChoiceComponent.make (wires cb) ... :56-66
//    - ChoiceComponentView (interactive) . :75-184  (local-first tap at :175-183)
//  New-user-turn bridge: OpenAIRealtimeSample/Conversation/ConversationModel.swift:384-390
//
//  THE interactive component — and the clearest proof of "model passes DATA, app owns
//  BEHAVIOR". The builder wires the view's `onPick` to `FactoryContext.onUserChoice`; the
//  model never ships a callback, only the option ids (data).
//
//  TURN-FLOW (realtime-delicate): the FIRST tap is LOCAL and INSTANT — the view marks the
//  chosen option, disables further taps, and calls `onPick(option.id)`. It does NOT wait
//  on the model and shows no spinner. The pick travels app-side and is sent as a NEW USER
//  TURN — NOT a second response on the render_component turn. (Wiring a 2nd response.create
//  here would break the one-response-per-turn invariant.)
//
...
extension ChoicePayload {
    /// Validate + clean: a non-empty prompt and at least one option with a usable id +
    /// label. Trims, drops blank/duplicate-id options, caps at 4. Returns nil when there's
    /// nothing usable so the builder throws → the mandatory fallback.
