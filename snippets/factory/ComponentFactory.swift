//
//  ComponentFactory.swift  (REDACTED / adapted for minimal-realtime-kit)
//  Source: OpenAIRealtimeSample/Factory/ComponentFactory.swift
//    - ComponentBuilder typealias ........ :22
//    - ComponentFactory (register/make) .. :24-50   (total fallback at :40-49)
//    - Logger.componentFactory ........... :52-58   (bundle id genericized below)
//    - DEBUG fallback-coverage probe ..... :60-232  (trimmed to the MVP three here)
//
//  The registry + front door for agent-driven UI. Builders are registered by
//  `ComponentID`; the app (NOT the model) owns this map. `make(_:context:)` NEVER throws
//  to the caller: an unregistered id returns a `FallbackComponentVC`, and a builder that
//  throws (malformed payload) is caught and also returns a `FallbackComponentVC`. That
//  total fallback is what makes free agent selection safe — the worst case is always a
//  small, safe card, never a crash or a wedged UI.
//
...
/// context. Throwing is the contract: a malformed payload throws, the factory catches it
/// and falls back.
...
@MainActor final class ComponentFactory {
    ...
    /// `register()` compiles fine and that id silently routes to the fallback at runtime.
    /// See `assertAllRegistered()` below for a cheap launch-time guard that turns the
    /// silent runtime trap into a DEBUG failure.
...
extension Logger {
    /// Shared log for the factory: registry decisions + every fallback. Replace the
    /// subsystem with your own bundle id.
    nonisolated static let componentFactory = Logger(
        subsystem: "com.example.realtimekit",   // REDACTED (was a real bundle id)
...
extension ComponentFactory {
    /// Fallback-coverage proof: exercise `make(...)` for a VALID and a MALFORMED request
    /// of each MVP component. The valid ones must return a real hosted body; the malformed
    /// ones must return a `FallbackComponentVC`. Prints a greppable marker AND logs it, so
    /// both the fallback path and every registration are provable headlessly (console
    /// capture is flaky on the sim). Inert unless a caller invokes it.
    static func runDemoComponentsProbe() {
        ...
        // Registered, but the required `metric` is missing → validated() nil → fallback.
